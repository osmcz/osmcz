...
L.Control.PhotoDBGui = L.Control.extend({
    ...
    $('#myModal').on('show.bs.modal', function() {
      ...
      $('#modalImg').on('load', function() {
        setTimeout(function() {
          ...
          $('#myModal').css(
            'margin-left',
            ($(window).width() - $('#myModal .modal-content').width()) / 2
          );
        }, 10);
      });
  ...
  _previewFile: function(e) {
    ...
    if (file.type.match(imageType)) {
      ...
      if (file) {
        ...
        if (
          file.size >
          $("#photoDB-upload-form input[name='MAX_FILE_SIZE']").val()
        ) {
          message.html(
            '<span class="glyphicon glyphicon-alert text-danger"></span> Soubor je moc velký!'
          );
          message.show();
        }
      } else {
        ...
        message.html(
          '<span class="glyphicon glyphicon-alert text-danger"></span> Povinné pole!'
        );
        message.show();
      }
    } else {
      ...
      preview.attr('style', 'display:none');
      message.html(
        '<span class="glyphicon glyphicon-alert text-danger"></span> Nepodporovaný typ souboru!'
      );
      message.show();
    }
  ...
  _readExif: function(e) {
    ...
    function noExif() {
      var message = $('#photoDB-upload-form #photoDB-img-message');
      message.html(
        '<span class="glyphicon glyphicon-alert text-danger"></span> Neobsahuje datum a čas v EXIF!'
      );
      message.show();
    }
    ...
    function badExifDateTime(val) {
      var message = $('#photoDB-upload-form #photoDB-img-message');
      message.html(
        '<span class="glyphicon glyphicon-alert text-danger"></span> Chybné datum a čas v EXIF!<br/>' +
          val
      );
      message.show();
    }
    ...
    var exif = EXIF.readFromBinaryFile(
      base64ToArrayBuffer(e.currentTarget.currentSrc)
    );
    ...
    if (
      exif.DateTime.search(/^1980:/) == 0 ||
      exif.DateTimeOriginal.search(/^1980:/) == 0
    ) {
    ...
    if (eLatRef != null && eLat != null && eLonRef != null && eLon != null) {
      ...
      message.html(
        'Pozici upravíte posunutím ukazatele nebo kliknutím do mapy.'
      );
      message.show();
    } else {
  ...
  _updateLatLonLabel: function(lat, lon) {
    ...
    if (!lat || lat == 0 || !lon || lon == 0) {
      ...
      elatlon.val(
        (lat * 1).toFixed(this._precision) +
          ', ' +
          (lon * 1).toFixed(this._precision)
      );
  ...
  _submitForm: function(e) {
    ...
    submitBtnIcon.attr(
      'class',
      'glyphicon glyphicon-refresh text-info gly-spin'
    );
        ...
        200: function(data) {
          ...
          if (result) {
            if (result[0] == 1) {
              ...
              // Error during upload
              toastr.error(
                'Fotografii se nepodařilo uložit na server.<br><em>Detail: </em>' +
                  result[1],
                'Chyba!',
                {
                  closeButton: true,
              ...
              submitBtnIcon.attr(
                'class',
                'glyphicon glyphicon-warning-sign text-danger'
              );
            }
          } else {
            // Unknown state of upload
            toastr.error(
              'Fotografii se nepodařilo  uložit na server.<br><em>Detail: </em>' +
                data,
              'Chyba!',
