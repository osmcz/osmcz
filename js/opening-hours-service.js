...
osmcz.openingHoursService.getHtml = function(v) {
  ...
  function getDayName(d) {
    ...
    if (idx in specialDays) return specialDays[idx];
  ...
  function splitByDays(oh_array) {
    ...
    for (i = 0; i < oh_array.length; i++) {
      ...
      if (formatedDayFrom == formatedDayTo) {
        // the same day
        ret[idx].push(
          formatTimeD(from) + '-' + formatTimeD(to).replace(/00:00/g, '24:00')
        );
      } else if (diffDays == 1)
        // over midnight
        ret[idx].push(
        ...
        while (formatedDayFrom != formatedDayTo) {
          ...
          if (formatedDayFrom != formatedDayTo)
            ...
            ret[idx].push(
              formatTimeHM(0, 0) +
                '-' +
                formatTimeD(to).replace(/00:00/g, '24:00')
            );
        }
      }
    }
  ...
  function formatWeek(aOH, dFrom, nDays, bVisible) {
    ...
    var clVisible = '';
    if (!bVisible) clVisible = 'class="oh-extended" style="display:none"';
  ...
  if (nextChangeDate)
    var diffChangeMin = Math.ceil((nextChangeDate - now) / 1000 / 60);
  // difference to next change of state in minutes
  else var diffChangeMin = -1;
  ...
  ret.push(
    " <a href=\"#\" onclick=\"if ($('.oh-extended').is(':visible')) {$('.oh-toggle').html('&#9660;');} else {$('.oh-toggle').html('&#9650;');} $('.oh-extended').toggle(200); return false\"><span class=\"oh-toggle\">&#9660;</span></a></h5>"
  );
  ret.push('<table>');
