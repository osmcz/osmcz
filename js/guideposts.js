...
osmcz.guideposts = function(map, baseLayers, overlays, controls, group) {
    ...
    onEachFeature: function(feature, layer) {
          ...
          if (tags.length > 0) {
            ...
            for (i = 0; i < tags.length; i++) {
              tags_content +=
                ...
                '?tag=' +
                tags[i] +
                '"><span id="hashtag" class="label label-info">' +
                tags[i].replace(/:$/, '') +
                '</span></a> ';
            }
            return tags_content;
      ...
      if (b.tags) {
        if (b.tags.indexOf('necitelne') > -1) {
          ...
          b.tags.indexOf('emergency') > -1 &&
          b.tags.indexOf('rozcestnik') == -1
        ) {
          ftype = 'emergency';
        } else if (
          (b.tags.indexOf('cyklo') > -1 || b.tags.indexOf('silnicni') > -1) &&
  ...
  map.on('click', function(e) {
    if (moving_flag) {
      if (!moving_marker) {
        ...
        update_sidebar(
          get_distance(moving_marker, position),
          position.lat,
          position.lng
        );
      } else {
        ...
        update_sidebar(
          get_distance(moving_marker, new_position),
          new_position.lat,
          new_position.lng
        );
  ...
  osmcz.guideposts.prototype.finish_moving = function() {
    if (moving_marker) {
      ...
      toastr.error('Nejprve prosím vyberte novou pozici.', 'Chyba!', {
        closeButton: true,
    ...
    $.ajax({
      ...
      .fail(function(jqXHR, textStatus, errorThrown) {
        ...
        if (status == 401) {
          need_api_auth = true;
          toastr.error('Nejste přihlášen(a).', 'Chyba!', {
            closeButton: true,
            positionClass: 'toast-bottom-center',
            timeOut: 0
          });
          ...
          toastr.error(
            'Uložení nové pozice se nepovedlo. ' + status + ': ' + statusText,
            'Chyba!',
            {
              closeButton: true,
              positionClass: 'toast-bottom-center',
