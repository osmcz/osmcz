...
osmcz.osmNotesLayer = L.LayerGroup.extend({
  ...
  onAdd: function(map) {
    ...
    map.on('viewreset', this._load, this).on('moveend', this._load, this);

    this._load();

    function pointToLayer(p) {
      return L.marker([p.geometry.coordinates[1], p.geometry.coordinates[0]], {
    ...
    function loadSuccess(resp) {
      for (var i = 0; i < resp.features.length; i++) {
        if (!this._loadedIds[resp.features[i].properties.id]) {
          resp.features[i].properties = this._template(
  ...
  onRemove: function(map) {
    ...
    map.off('viewreset', this._load, this).off('moveend', this._load, this);
  },
  ...
  _icon: function(fp) {
    ...
    return L.icon({
      iconUrl:
        'http://a.tiles.mapbox.com/v3/marker/' +
        'pin-' +
        size.charAt(0) +
        symbol +
        '+' +
  ...
  _template: function(p) {
    ...
    p.title =
      (p.status == 'closed' ? 'Vyřešená poznámka' : 'Poznámka') +
      ' <a href="http://www.openstreetmap.org/browse/note/' +
      p.id +
      '">#' +
      p.id +
      '</a>';
    ...
    for (var i = 0; i < p.comments.length; i++) {
      ...
      p.description +=
        ...
        '</div>';
      p.description +=
        '<div class="comment-text">' + p.comments[i].html + '</div>';
    }
