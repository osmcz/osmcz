...
function initmap() {
  ...
  L.control
    .condensedAttribution({
      ...
      prefix:
        "<a href='https://github.com/osmcz/osmcz' title='Projekt na Githubu'><img src='https://github.com/favicon.ico' width='10' style='margin-right:1ex'>osmcz-app</a> " +
        OSMCZ_APP_VERSION
    })
    .addTo(map);
  ...
  L1: for (group in baseLayers) {
    for (layer in baseLayers[group]) {
      if (
        baseLayers[group][layer].options &&
        baseLayers[group][layer].options.osmczDefaultLayer
      ) {
  ...
  // -------------------- modules --------------------
  guideposts = new osmcz.guideposts(
    map,
    baseLayers,
    overlays,
    controls,
  ...
  if (window.__npress_page_osm_params__)
    Object.assign(params, window.__npress_page_osm_params__);
...
function loadPoiFromParams(params) {
  if (params.object) osmcz.poiPopup.load(params.object);
...
function updateLayersFromCode(codedString) {
  ...
  if (!codedString || !codedString.match(/[a-z]/)) {
    ...
    for (group in baseLayers) {
      for (layer in baseLayers[group]) {
        if (setLayer(layer, baseLayers[group][layer])) baseLayerSelected = true;
      }
    }
    if (!baseLayerSelected) map.addLayer(osmcz.defaultLayer);
  }
