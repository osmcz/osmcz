...
osmcz.poiPopup.load = function(object) {
  ...
  $.ajax({
    url:
      ...
      OSM.apiUrl({ type: object.type, id: object.id }),
    dataType: 'xml',
    jsonp: false,
    global: false,
    success: function(data) {
      ...
      if (feature.geometry.type !== 'Point') {
        // take centroid of a Polygon
        var centroid;
        try {
          centroid = L.polygon(feature.geometry.coordinates[0])
            .getBounds()
            .getCenter();
        } catch (e) {
          centroid = L.polygon(feature.geometry.coordinates)
            .getBounds()
            .getCenter();
        }
      ...
      poiSidebar.setContent(
        osmcz.poiPopup.getHtml(feature, icon.options.iconUrl)
      );
      poiSidebar.show();
...
osmcz.poiPopup.open = function(feature, icon) {
  //currently from active-layer
...
osmcz.poiPopup.close = function() {
  ...
  var path =
    location.host === 'openstreetmap.cz' || location.host === 'osm.localhost'
      ? '/'
      : location.pathname;
...
osmcz.poiPopup.getHtml = function(feature, icon, embedded) {
  ...
  if (!embedded) {
    ...
    tpl.push(
      '<span class="h4-text">' +
        (feature.properties.tags.name || 'Bod zájmu') +
        '</span>'
    );
    tpl.push('</h4>');
  }
  ...
  $.each(feature.properties.tags, function(k, v) {
    ...
    if (k.match(/^name:/)) name[k] = v;
    else if (k.match(/^payment:/)) payment[k] = v;
    else if (k.match(/^contact:/) || k.match(/^phone/) || k.match(/^email/))
      contact[k] = v;
    else if (k.match(/^building/)) building[k] = v;
    else if (k.match(/^opening_hours$/)) openingHours = v;
    else if (k.match(/^addr:|^ref:ruian:/))
      //skip TODO show in expert mode
      undefined;
    else general.push({ k: k, v: v });

    // global flags
    if (k.match(/^wikipedia/)) {
      // Prefer wikipedia over wikipedia:cs over first one
      if (k.match(/^wikipedia$/)) wikipedia = { k: k, v: v };
      else if (
        k.match(/^wikipedia:cs$/) &&
      ...
      wikimedia = { k: k, v: v };
    else if (k.match(/^wikidata/)) wikidata = { k: k, v: v };
  ...
  if (general.length) {
    for (var i in general) {
      ...
      // Katalog národního památkového ústavu
        ...
        tpl.push(
          '<a href="http://pamatkovykatalog.cz/?mode=parametric&isProtected=1&presenter=ElementsResults&indexId=' +
            encodeURIComponent(v) +
            '">' +
            v +
            '</a>'
        );
      // @TODO: upravit, až bude HTTPS verze
      // Just standard url
      else
        tpl.push(
          v.match(/^https?:\/\/.+/) ? '<a href="' + v + '">' + v + '</a>' : v
        );
      tpl.push('<br>');
    }
  }
    ...
    if (Object.keys(obj).length) {
      hide &&
        tpl.push(
          '<p><b>' +
            label +
            '</b> <a href="#" onclick="$(this).parent().hide().next().show();return false">zobrazit</a></p>'
        );
      ...
      hide &&
        tpl.push(
          '<h5>' +
            label +
            ' <a href="#" onclick="$(this).parent().parent().hide().prev().show();return false">skrýt</a></h5>'
        );
  ...
  try {
    ...
    tpl.push(
      '<b>opening_hours</b> = ' +
        openingHours +
        '<br> <em>(Nepodařilo se naparsovat: ' +
        err +
        ')</em>'
    );
  }
  ...
  tpl.push(
    '<div class="osmid"><a href="https://osmap.cz/' +
      osm_type +
      '/' +
      id +
      '">osmap.cz/' +
  ...
  if (wikimedia.k || wikipedia.k || wikidata.k) {
    var url;
    if (wikidata.k) url = getWikiApiUrl(wikidata);
    else if (wikimedia.k) url = getWikiApiUrl(wikimedia);
    else url = getWikiApiUrl(wikipedia);
      ...
      success: function(data) {
        ...
        if (replyType == 'wikimedia') {
          ...
          if (!reply.k) {
            ...
            // try other tags
            if (wikimedia.k) reply = wikimedia;
            else if (wikipedia.k) reply = wikipedia;
          }
  ...
  function showWikimediaCommons() {
    // Template
    var wcmTpl =
      '<h5><a href="https://commons.wikimedia.org/">' +
      '<img class="commons_logo" src="' +
      osmcz.basePath +
      'img/commons_logo.png" height="24"></a>' +
      'Foto z Wikipedie</h5>' +
    ...
    if (
      id == wcm.attr('data-osm-id') &&
      (feature.wikimedia || feature.wikipedia)
    ) {
      if (feature.wikimedia) {
        var k = Object.keys(feature.wikimedia.query.pages)[0];
        if (!feature.wikimedia.query.pages[k].imageinfo.length) return;
        var descriptionshorturl =
          feature.wikimedia.query.pages[k].imageinfo[0].descriptionshorturl;
        var thumburl = feature.wikimedia.query.pages[k].imageinfo[0].thumburl;
      } else {
        var k = Object.keys(feature.wikipedia.query.pages)[0];
        if (!feature.wikipedia.query.pages[k].pageimage) return;
        var descriptionshorturl =
          'https://commons.wikimedia.org/wiki/File:' +
          feature.wikipedia.query.pages[k].pageimage;
        var thumburl = feature.wikipedia.query.pages[k].thumbnail.source;
      }
      wcm.html(
        wcmTpl
          .replace(/_thumburl/g, thumburl)
          .replace(/_descriptionshorturl/g, descriptionshorturl)
      );
    }
  }
  ...
  var gpTpl =
    ...
    ' <a href="' +
    osmcz.photoDbUrl +
    '?id=_id" target="_blank" class="btn btn-default btn-xs">' +
  ...
  function showGuidepost() {
    ...
    if (id == gp.attr('data-osm-id') && feature.guidepost) {
      ...
      if (!feature.guidepost.features.length) return;
      ...
      gp.html(
        gpTpl
          .replace(/_autor/g, autor)
          .replace(/_imgUrl/g, fullImgUrl)
          .replace(/_id/g, gpostId)
      );
      ...
      tb.onerror = function() {
        $('#thumbnailLoadSpinner').html(
          '<br><span class="glyphicon glyphicon-picture bigger semigrey thumbnail crossed" title="Náhled není k dispozici."><span><br>'
        );
  ...
  if (osmcz.permanentlyDisplayed) {
    if (!feature.wikimedia && !feature.wikipedia && !feature.guidepost) {
      ...
      if (feature.mapillary) {
        ...
        $.ajax({
          url:
            'https://api.mapillary.com/v1/im/close?lat=' +
            lat +
            '&lon=' +
            lon +
            '&distance=30&limit=1',
  ...
  var mpTpl =
    '<h5>Nejbližší street-view</h5>' +
    '<a href="https://www.mapillary.com/map/im/_key/photo">' +
    '<img src="' +
    osmcz.fakeHttps +
    'images.mapillary.com/_key/thumb-320.jpg" height="187">' + //@todo https viz https://github.com/mapillary/mapillary_issues/issues/2419
    '</a>';
  ...
  function getWikiApiUrl(we) {
    if (we.k == 'wikimedia_commons') {
      // wikimedia commons tag
      return (
        'https://commons.wikimedia.org/w/api.php?action=query&prop=imageinfo&iiprop=url&iiurlwidth=240&format=json' +
        '&titles=' +
        encodeURIComponent(we.v)
      );
    } else if (we.k == 'wikidata') {
      // wikidata tag
      return (
        'https://www.wikidata.org/w/api.php?action=wbgetclaims&property=P18&format=json' +
        '&entity=' +
        encodeURIComponent(we.v)
      );
    } else {
      ...
      if (country === we.v) country = 'en';
      return (
        'https://' +
        country +
        '.wikipedia.org/w/api.php?action=query&prop=pageimages&pithumbsize=240&format=json' +
        '&titles=' +
        encodeURIComponent(we.v)
      );
    }
