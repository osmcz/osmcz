...
osmcz.controls = function(map, baseLayers, overlays, layersPanel, controls) {
  ...
  controls.layers = osmcz
    .layerSwitcher(baseLayers, overlays, layersPanel, {
      priorityGroups: ['Základní', 'Letecké', 'Informace']
    })
    .addTo(map);
  ...
  controls.fileLayerLoad.loader.on('data:error', function(e) {
    toastr.error(
      'Soubor se nepodařilo načíst.<br>(<i>' + e.error.message + '</i>)',
      'Chyba',
      {
        closeButton: true,
        positionClass: 'toast-bottom-center'
