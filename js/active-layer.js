    ...
    },
    {
      ...
      pointToLayer: function(feature, latlng) {
        return L.marker(latlng, {
          icon: osmcz.iconsService.get(feature.properties.tags)
        });
      },
        ...
        if (!(layer instanceof L.Point)) {
          layer.on('click', function(event) {
            if (
              ...
              osmcz.poiPopup.open(
                event.target.feature,
                event.target.options.icon.options.iconUrl
              );
          ...
          layer.on('mouseover', function(event) {
            if (osmcz.permanentlyDisplayed || osmcz.sidebar.isVisible()) return;
