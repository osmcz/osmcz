  ...
  var layer_gpcheck = new L.GeoJSON(null, {
    onEachFeature: function(feature, layer) {
      ...
      if (feature.properties.class == 'tourism') {
        html_content +=
          '<span class="glyphicon glyphicon-remove text-danger"></span> jen tourism=info<br/>';
      }
      html_content += '</div>';
      ...
      html_content +=
        '<br/><div class="osmid"><a href="https://osmap.cz/node/' +
        feature.id +
        '">osmap.cz/node/' +
        feature.id +
        '</a>' +
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (osmid == gc.attr('gp-check-id')) {
          gc.html(
            '<h6>Data v OSM</h6>' + osmcz.poiPopup.getHtml(feature, null, true)
          );
          if ('ref' in tags) {
