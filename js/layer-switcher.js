...
osmcz.LayerSwitcher = L.Control.extend({
  ...
  _prepareLayout: function() {
    ...
    if (this.options.collapsed) {
      if (!L.Browser.android) {
        L.DomEvent.on(container, 'click', this._expand, this) //osmcz
          .on(container, 'mouseout', this._collapse, this);
      }

      if (!this._layersLink) {
        this._layersLink = L.DomUtil.create(
          'a',
          className + '-toggle',
          container
        );
        ...
        if (L.Browser.touch) {
          L.DomEvent.on(this._layersLink, 'click', L.DomEvent.stop).on(
            this._layersLink,
            'click',
            this._expand,
            this
    ...
    // Prepare layout
    this._separatorMain = L.DomUtil.create(
      'div',
      className + '-separator',
      form
    );
    ...
    this._btnExpandAll.setAttribute(
      'onclick',
      'controls.layers.expandAllGroups();'
    );
    this._btnExpandAll.innerHTML =
      '<i class="glyphicon glyphicon-eject flip"></i>';
    ...
    this._btnCollapseAll.setAttribute(
      'onclick',
      'controls.layers.collapseAllGroups();'
    );
    this._btnCollapseAll.innerHTML =
    ...
    this._btnSetting.className =
      'btn btn-secondary  btn-default btn-xs pull-right hidden';
    ...
    labelBasic.className =
      'btn btn-default' + (this._mode == 'basic' ? ' active' : '');
    labelBasic.setAttribute(
      'onclick',
      'controls.layers._switchLayerMode("basic")'
    );
    ...
    lssetup.innerHTML =
      '<h3>Nastavení</h3><div>Tady bude nějaké nastavení.</div>';
    this._form.appendChild(lssetup);

    this._separatorBtGroup = L.DomUtil.create(
      'div',
      className + '-separator',
      form
    );
    ...
    this._baseSeparator = L.DomUtil.create(
      'div',
      className + '-separator',
      basicModeContainer
    );
  ...
  _addLayer: function(layer, name, overlay, group) {
    ...
    if (!overlay && layer.options.osmczDefaultLayer) this._defaultLayer = layer;
    ...
    if (this._groups[group]) {
      if (
        ...
        if (Cookies.get('_ls_expanded_groups') == null) {
          ...
          Cookies.set('_ls_expanded_groups', expandCookie.join('|'), {
            expires: 90
          });
        }
      }
